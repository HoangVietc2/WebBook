...
const hanleCPayment = async function (req, res) {
  try {
    ...
    if (!req.body.Data.selectedPaymentMethod) {
      res.status(403).json("Vui Lòng chọn phương thức thanh toán");
    } else {
      let status = 1;
      ...
      if (!fullname || !address || !phone) {
        ...
        if (payment == 2) {
          ...
          Payment(amount_total, fullname, async (parsedData) => {
            await yourUpdateFunction(
              id_user,
              fullname,
              address,
...
// Payment function
function Payment(amount_total, fullname, callback) {
  ...
  var redirectUrl = "http://127.0.0.1:5500/HCI/order.html";
  var ipnUrl = "http://127.0.0.1:5500/HCI/thankouypayment.html";
  var amount = amount_total;
  var requestType = "captureWallet";
  var extraData = fullname;
    ...
    response.on("end", () => {
      try {
        ...
        if (response.statusCode === 200) {
          ...
          console.log(parsedData.signature);
          callback(parsedData);
        } else {
...
async function yourUpdateFunction(
  id_user,
  fullname,
  address,
  status,
...
const HandleInserOrder = async (req, res) => {
  try {
    console.log(req.body);
    res.status(200).json("Bạn đã giao dịch thành công");
  } catch (error) {
    console.error(error);
    res.status(500).send("Internal Server Error");
  }
...
module.exports = { hanleCPayment, HandleInserOrder };
