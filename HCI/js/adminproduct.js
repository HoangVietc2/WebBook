    ...
    .done(function (response) {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        response.data.forEach((value, idx) => {
          ...
          let html = `<tr>
            <td>${stt}</td>
            <td><img class="img-fluid rounded" src="https://lh3.googleusercontent.com/d/${value.img}" alt=""></td>
            <td>${value.ten}</td>
...
function showAuthor() {
  $.ajax({
    ...
    .done(function (data) {
      let selectOption = "";
      data.forEach((value) => {
        selectOption += `<option value="${value.id_company}">${value.namecompany}</option>`;
      });
    ...
    .done(function (data) {
      ...
      if (data && data.length > 0) {
        ...
        data.forEach(function (value) {
          ...
          let selectedauthor = `<option value = "${value.id_company}" selected>${value.namecompany}</option>`;
          $("#giaup").val(value.gia);
